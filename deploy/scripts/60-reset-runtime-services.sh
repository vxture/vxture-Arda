#!/usr/bin/env bash
# Reset this stack's Arda deployment.
#
# Default (soft): stops containers and frees ports for a clean re-deploy.
#   Data is preserved.
#
# --full: destroys all runtime data including Redis state. Requires typing YES
#   to confirm. Use before reprovisioning.
#
# Usage:
...
MODE="${1:-}"
CONTAINERS=("${PROJECT_NAME}-app" "${PROJECT_NAME}-redis")
PORTS=("${APP_PUBLISH_PORT:-3230}")
...
log_banner "Arda - Soft Reset"
log_info "Stops containers and frees ports. Data is preserved."
echo ""
