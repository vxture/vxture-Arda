...
- **有效 arda 权益的两种来源，取用路径一视同仁**（`ADR-11` §11.3 就高合并）：
  ...
  - **附带（bundled，旧称 standard）**：workspace **未单独订阅 arda**，但其订阅的某 agent Plan 附带了 arda 数据底座（`billing=bundled_free`，能力档 = `free`）——arda 从**后台**支撑该 agent 的数据取用，**不提供 arda 产品 UI**。这正是"agent 需要数据支持"的场景：C2 对该 workspace 返回 `status=active` + `tier=free` 的权益，取用闸门放行。
