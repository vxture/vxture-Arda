...
**一句话职责（分界线）**：arda 只管**数据本身**的六件事——**资产、标准、治理、质量、服务、更新（保鲜）**；**不管数据怎么用、在哪用**（用是消费方的事）。这条是 arda 与所有消费方（含智能体）的边界。

> 六件事 → 板块映射：资产 → `assets`；标准 / 质量 / 治理（含血缘 / 安全策略）→ `governance`；服务 → `services`；更新（数据源同步 / 保鲜）→ `integration`；`admin`（密钥 / 审计）为支撑面，服务于上述六件事的可管可审。
